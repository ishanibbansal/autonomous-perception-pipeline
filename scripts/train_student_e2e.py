...
def train_student_e2e(args):
    ...
    print(f"Initializing Phase 3: E2E Fine-Tuning on {device}")
    ...
    # 2. Load Phase 2 Temporal Student
    student = StudentBEVDetector(use_temporal=True).to(device)
    print(f"Loading Phase 2 Temporal Backbone from {args.temporal_ckpt}...")
    s_checkpoint = torch.load(args.temporal_ckpt, map_location=device)
    student.load_state_dict(s_checkpoint['model_state_dict'], strict=True)

    # 3. UNFREEZE ALL LAYERS for E2E Fine-Tuning
    print("Unfreezing spatial backbone for E2E learning...")
    ...
    optimizer = optim.AdamW([
        ...
        {'params': student.reduce_channel.parameters(), 'lr': 1e-5},
        {'params': student.view_transformer.parameters(), 'lr': 1e-5},
        {'params': student.adaptation.parameters(), 'lr': 1e-5},
        {'params': student.se_block.parameters(), 'lr': 1e-5},
        {'params': student.temporal_fusion.parameters(), 'lr': 5e-5},
        {'params': student.bev_head.parameters(), 'lr': 5e-5}
    ], weight_decay=1e-4)                                 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--temporal_ckpt', type=str, default='best_temporal_student_checkpoint.pt', help='Phase 2 weights')
